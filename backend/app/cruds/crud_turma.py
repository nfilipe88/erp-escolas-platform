...
def get_turmas(db: Session, skip: int = 0, limit: int = 100, escola_id: int = None):
...
def get_turmas_by_escola(db: Session, escola_id: int, skip: int = 0, limit: int = 100):
    return db.query(models.Turma).filter(models.Turma.escola_id == escola_id).offset(skip).limit(limit).all()

def get_turma(db: Session, turma_id: int, escola_id: int = None):
